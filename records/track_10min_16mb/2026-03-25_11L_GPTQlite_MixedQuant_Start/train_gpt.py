...
class Hyperparameters:
    ...
    gptq_clip_percentiles = tuple(
        float(x)
        for x in os.environ.get("GPTQ_CLIP_PERCENTILES", "0.999,0.9995,0.9999,0.99999,1.0").split(",")
        if x
    )
    quant_int6_categories = tuple(
        x.strip()
        for x in os.environ.get("QUANT_INT6_CATEGORIES", "mlp,attn").split(",")
        if x.strip()
    )
    quant_int8_block_patterns = tuple(
        x.strip()
        for x in os.environ.get("QUANT_INT8_BLOCK_PATTERNS", "").split(",")
        if x.strip()
    )
    quant_int8_last_k_layers = int(os.environ.get("QUANT_INT8_LAST_K_LAYERS", 0))
    quant_int6_clip_range = int(os.environ.get("QUANT_INT6_CLIP_RANGE", 31))
...
def _block_layer_index(name: str) -> int | None:
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    clip_percentiles: tuple[float, ...],
    int8_block_patterns: tuple[str, ...],
    int8_last_k_layers: int,
    int6_clip_range: int,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        layer_idx = _block_layer_index(name)
        force_int8 = any(pattern in name for pattern in int8_block_patterns)
        if int8_last_k_layers > 0 and layer_idx is not None and layer_idx >= num_layers_total - int8_last_k_layers:
            force_int8 = True
        if cat in int6_cats and t.ndim >= 1 and not force_int8:
            q, s = quantize_int6_per_row(t, clip_percentiles=clip_percentiles, clip_range=int6_clip_range)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    int6_cats = set(args.quant_int6_categories)
    log0(
        f"quant_config:int6_cats:{sorted(int6_cats)} clip_percentiles:{args.gptq_clip_percentiles} "
        f"int8_last_k_layers:{args.quant_int8_last_k_layers} int8_block_patterns:{args.quant_int8_block_patterns} "
        f"int6_clip_range:{args.quant_int6_clip_range}"
    )
    quant_result, quant_meta = mixed_quantize_int6(
        sd_cpu,
        int6_cats=int6_cats,
        clip_percentiles=args.gptq_clip_percentiles,
        int8_block_patterns=args.quant_int8_block_patterns,
        int8_last_k_layers=args.quant_int8_last_k_layers,
        int6_clip_range=args.quant_int6_clip_range,
    )
